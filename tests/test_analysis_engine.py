...
class TestAnalysisEngineHistoricalData(unittest.TestCase):
    ...
    def test_generate_signals_with_sufficient_data(self): # Removed mock_fetch_fund_flow
        mock_stock_code = "000001"
        mock_data_25_days = create_mock_stock_data(25)

        # Removed mock fund flow configuration and assertion
        ...
        # Removed mock_fetch_fund_flow.assert_called_once_with(...)
        ...
        # Removed fund_flow_map_for_assertion
        ...
        for i, item in enumerate(historical_data['ohlcv']):
            ...
            # Removed assertions for 'net_inflow' and 'net_inflow_pct'
            self.assertNotIn('net_inflow', item) 
            self.assertNotIn('net_inflow_pct', item)

            # Check turnover and change_pct based on create_mock_stock_data logic
            original_data_index = 5 + i 
            ...
            # Removed expected_fund_flow checks
    ...
    def test_generate_signals_with_insufficient_data(self): # Removed mock_fetch_fund_flow
        mock_stock_code = "000001"
        mock_data_10_days = create_mock_stock_data(10)
        # Removed mock_fetch_fund_flow.return_value = []
        ...
        for i, item in enumerate(historical_data['ohlcv']):
            self.assertIn('turnover', item)
            self.assertEqual(item['turnover'], 1000000.0 + (i + 1) * 10000) 
            self.assertIn('change_pct', item)
    ...
    def test_historical_data_default_on_error(self): # Removed mock_fetch_fund_flow
        mock_stock_code = "000001"
        ...
        # Removed mock_fetch_fund_flow.assert_called_once_with(...)
        # or mock_fetch_fund_flow.assert_not_called()
